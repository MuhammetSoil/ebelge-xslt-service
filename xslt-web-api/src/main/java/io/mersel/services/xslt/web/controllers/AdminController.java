...
public class AdminController {
    ...
    /**
     * Auto-generated dizinindeki belirli bir dosyanın içeriğini döndürür.
     */
    @GetMapping("/auto-generated/content")
    @Operation(
            summary = "Auto-generated dosya içeriğini getir",
            description = "Belirtilen auto-generated dosyanın metin içeriğini döndürür. "
                    + "Path parametresi 'auto-generated/' prefix'i ile birlikte verilmelidir "
                    + "(ör: auto-generated/schematron-rules/UBLTR_MAIN_global_custom.xml)."
    )
    public ResponseEntity<?> getAutoGeneratedContent(@RequestParam("path") String path) {
        if (path == null || path.isBlank()) {
            return ResponseEntity.badRequest().body(
                    new ErrorResponse("Geçersiz path", "Dosya yolu belirtilmedi"));
        }

        if (!path.startsWith("auto-generated/")) {
            return ResponseEntity.badRequest().body(
                    new ErrorResponse("Geçersiz path", "Sadece auto-generated/ dizinindeki dosyalar okunabilir"));
        }

        if (path.contains("..") || path.contains("//")) {
            return ResponseEntity.badRequest().body(
                    new ErrorResponse("Geçersiz path", "Path traversal desteklenmez"));
        }

        try {
            if (!assetManager.assetExists(path)) {
                return ResponseEntity.status(404).body(
                        new ErrorResponse("Dosya bulunamadı", "Belirtilen dosya mevcut değil: " + path));
            }

            var content = new String(assetManager.readAssetBytes(path), java.nio.charset.StandardCharsets.UTF_8);
            String fileName = path.substring(path.lastIndexOf('/') + 1);
            String language = fileName.endsWith(".xsl") || fileName.endsWith(".xslt") ? "xml" : "xml";

            return ResponseEntity.ok(new AutoGeneratedContentResponse(
                    path, fileName, language, content, content.length()));
        } catch (IOException e) {
            log.error("Auto-generated dosya okuma hatası: {} — {}", path, e.getMessage());
            return ResponseEntity.internalServerError().body(
                    new ErrorResponse("Dosya okunamadı", e.getMessage()));
        }
    }
    ...
    record AutoGeneratedContentResponse(String path, String fileName, String language, String content, long size) {}
